...
# Each entry is "graph_type:edge_factor". nonlinear.py calculates s0 as d times
        ...
        for graph_setting in "${GRAPH_SETTINGS[@]}"; do
            ...
            for sem_type in "${SEM_TYPES[@]}"; do
                for prior_type in "${PRIOR_TYPES[@]}"; do
                    echo "Running seed=${seed} d=${d} graph=${graph_type}${edge_factor} sem=${sem_type} prior=${prior_type} rate=${PRIOR_RATE}"

                    output_dir="${PROJECT_ROOT}/nonlinear_${prior_type}"
                    log_dir="${output_dir}/log"
                    result_stem="nonlinear_${prior_type}_${graph_type}${edge_factor}_d${d}_${sem_type}_rate${PRIOR_RATE}_seed${seed}"
                    result_file="${output_dir}/${result_stem}.json"
                    log_file="${log_dir}/${result_stem}.log"
                    mkdir -p "${output_dir}" "${log_dir}"

                    "${PYTHON_BIN}" -m prior_notears.nonlinear \
                        --seed "${seed}" \
                        --num_nodes "${d}" \
...
echo "All nonlinear experiments completed."
